...
public sealed class JsonMetadataStore : IMetadataStorePort
{
    ...
    public JsonMetadataStore(string metadataPath)
    {
    ...
    public async Task<IReadOnlyList<Segment>> GetUnarchivedSegmentsAsync(int limit, CancellationToken ct)
    {
        ...
        {
            return segments
                .Where(s => string.IsNullOrWhiteSpace(s.AdlsPath))
    ...
    public async Task MarkSegmentArchivedAsync(Guid segmentId, string adlsPath, DateTimeOffset archivedAt, CancellationToken ct)
    {
        ...
        try
        {
            var seg = segments.FirstOrDefault(s => s.SegmentId == segmentId);
            if (seg is null) return;

            seg.AdlsPath = adlsPath;
            seg.ArchivedAt = archivedAt;
            seg.UpdatedAt = archivedAt;
