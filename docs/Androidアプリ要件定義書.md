...
- 対応解像度/フレームレート
	...
	- ユーザーは変更できないようにする。設定でアプリケーションのビルド時に変更できるようにする。
- 基本機能（プレビュー/録画/再生）はオフライン動作可能であること
- 録画中は、端末のバックグラウンド制約で停止しないよう Foreground Service を前提とする
	- 録画中は常駐通知を表示する（ユーザーが「録画中」であることを常に把握できる）
	- 画面OFFでも録画を継続できること
	- 省電力設定（Doze/バッテリー最適化等）により録画が停止し得る端末があるため、必要な注意喚起を行うこと
...
- 録画中は Foreground Service と常駐通知により、画面OFF/バックグラウンドでも継続できること
- 録画中に録画済みの動画を再生しても、録画（セグメント生成）が停止しないこと
...
- 工程は、Web APIから取得する（工程マスタ）
	...
	- Web APIに接続できない場合でも、現場が停止しないための継続策を提供する（最小）
		- 最終取得の工程マスタを端末内にキャッシュし、有効期限（TTL）内であれば「暫定選択モード」として利用できる
		- キャッシュが無い/期限切れの場合は、暫定工程として `UNKNOWN` を選択できる（監査上の扱いは運用で確定）
		- 暫定選択で開始した場合は、画面上で暫定であることを明示する
...
- 未割当セグメントも一覧で確認できる
- 録画中（確定前）のセグメントは一覧に表示してもよいが、再生対象にはしない（再生不可とする）
- 並び順
	- 作業一覧: 作業開始日時の新しい順
...
- 全画面表示への切り替え/解除ができる
- 画面回転時も再生を継続する
- 録画中でも録画済みファイルを再生でき、再生操作によって録画（セグメント生成）が停止しないこと
- 再生対象は確定済み（録画停止/分割によりファイル確定済み）のセグメントに限定し、録画中（確定前）および確定中のセグメントは再生不可とする
...
- 録画中に録画済みの動画を再生しても、録画が停止しない
- 録画中（確定前）および確定中のセグメントは再生不可であり、確定済みセグメントのみ再生できる
