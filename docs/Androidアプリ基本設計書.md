...
- Web API（工程マスタ）の詳細仕様（URL/レスポンス形式/認証 等）は別資料で定義する
...
### 7.2 工程マスタ取得
- Web API から工程マスタを取得し、ユーザーが選択する
- 取得できない場合は、工程が確定できないため作業開始は成立しない（理由を画面に明示）
