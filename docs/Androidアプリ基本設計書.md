...
現状実装との整合:
- 既存実装は `Movies/UVC` への保存、Media3 による再生、HEVC+AAC の mux を行っている。将来要件（作業紐づけ/アップロード/削除）を追加しても、録画・再生の基本経路は維持する。
...
- 端末内の録画ファイル再生
- 再生/一時停止/シーク/全画面
	- 録画中でも録画済み（確定済み）ファイルを再生できる
	- 録画中（確定前）および確定中のセグメントは再生対象外（再生不可）とする
...
- 録画中に再生画面へ遷移し録画済みファイルを再生しても、RecordingState は原則として維持し、録画（セグメント生成）を停止しない
	- ただし RecordingState=ON/FINALIZING の「現在録画中（確定前/確定中）」セグメントは、エラー防止のため再生対象外（再生不可）とする
- 録画開始/停止: ユーザー操作、または容量不足/エラーによる停止

- 録画のバックグラウンド継続は Foreground Service を前提とし、要件・注意喚起を含めた方針は「14.4 バックグラウンド録画（Foreground Service）」に定義する
...
接続先と認証:
- 実際のホスト/スキーム/認証方式は運用・別資料で確定する。Android側は URL/認証設定を差し替え可能（疎結合）な実装とする。
...
- 録画中でも録画済みファイルを再生でき、再生操作が録画（セグメント生成）を停止しない（録画は Foreground Service を前提に継続）
- 再生対象は確定済みセグメントに限定し、録画中（確定前）および確定中（FINALIZING）のセグメントは再生不可とする
...
識別子の扱い（冪等・重複判定）:
- segmentUuid は録画セグメント生成時に端末で発行し、その後の後追い紐づけや segmentIndex 再採番が行われても不変とする。
- サーバー側の冪等処理・重複判定・ログ相関の中心は segmentUuid とする（segmentIndex は後追い紐づけ等で変動し得る）。
- メタデータの送信方法（HTTPヘッダ/別API/フォーム等）はサーバー仕様に合わせる。
...
工程マスタキャッシュ（7.2 の挙動に使用）
- 工程一覧キャッシュ: `items`（id/name の配列）
- 取得時刻: `fetchedAt`
...
- 録画中に録画済みファイルを再生しても録画が停止しない
- 録画中（確定前/確定中）のセグメントは再生不可であり、確定済みセグメントのみ再生できる
- 再生可能/不可時の挙動（クラッシュしない）
- 空き容量閾値時の自動削除順序と録画停止条件（12.2〜12.3）
- Wi‑Fi限定の自動アップロードと再開（11.1〜11.4）
