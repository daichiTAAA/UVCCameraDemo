...
flowchart LR
	...
	RP --> T[tusd]
	T -->|hook: upload completed| W[webapp (ASP.NET)]
	W --> DB[(PostgreSQL)]
	U[Browser] -->|UI + /api| RP
	RP --> W
...
- `TUSD_HOOK_HTTP_URL=http://webapp:8080/api/tusd/hooks/completed`（例）
...
services:
	...
	webapp:
		image: <registry>/uvccamera-webapp:<tag>
		environment:
			- ASPNETCORE_URLS=http://0.0.0.0:8080
			- ConnectionStrings__Main=Host=postgres;Port=5432;Database=uvc;Username=...;Password=...
			- Storage__LocalRoot=/data/videos
	...
	tusd:
		...
		command: [
			...
			"-base-path=/files/",
			"-hooks-http=http://webapp:8080/api/tusd/hooks/completed"
		]
		volumes:
...
- `videos` は `webapp` と `tusd` で共有する（受信→確定→配信）
...
DBマイグレーションが必要な場合:
- `webapp` 起動時に自動適用するか、別途 `migrate` ジョブとして一回実行する（実装で確定）
...
最低限の確認:
- `tusd` の開始/完了/失敗
- `webapp` の例外ログ（`segmentUuid/segmentId/workId` 等の相関情報）
...
- tusアップロードが成功し、完了フックが `webapp` に到達する
...
## 12. 未確定事項（実装で確定する）
- `webapp` の実装言語/フレームワーク確定（本書は ASP.NET を前提）
